...
public class SlotVespeneCondenserOutput  extends Slot
{
    /** The player that is using the GUI where this slot resides. */
    private final EntityPlayer thePlayer;
    private int numVespeneCondenserOutput;

    public SlotVespeneCondenserOutput(EntityPlayer parPlayer, IInventory parIInventory, int parSlotIndex, int parXDisplayPosition, int parYDisplayPosition)
    {
    ...
    public ItemStack decrStackSize(int parAmount)
    {
        if (getHasStack())
        {
            numVespeneCondenserOutput += Math.min(parAmount, getStack().stackSize);
        }
    ...
    {
        numVespeneCondenserOutput += parAmountGround;
    ...
    protected void onCrafting(ItemStack parItemStack)
    {
        ...
        {
            int expEarned = numVespeneCondenserOutput;
            float expFactor = RecipesVespeneCondenser.instance().getVespeneCondenserExperience(parItemStack);
