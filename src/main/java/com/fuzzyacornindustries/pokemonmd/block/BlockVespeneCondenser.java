...
public class BlockVespeneCondenser extends BlockContainer implements IPylonPowered
{
    private static boolean hasTileEntity;

	public static boolean isPowered;
	
	private static final String blockName = "vespenecondenser";
    ...
    {
        return Item.getItemFromBlock(PokemonMDBlocks.vespeneCondenser);
    }
          ...
          EntityPlayer parPlayer, EnumFacing parSide, float hitX, float hitY, float hitZ)
    {
        ...
        {
            parPlayer.openGui(PokemonMD.instance, PokemonMD.GUI_ENUM.VERDANT_PURIFIER.ordinal(), 
                  parWorld, parBlockPos.getX(), parBlockPos.getY(), parBlockPos.getZ()); 
        }
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state)
    {
        if (!hasTileEntity)
        {
            ...
            {
                InventoryHelper.dropInventoryItems(worldIn, pos, (TileEntityVespeneCondenser)tileentity);
